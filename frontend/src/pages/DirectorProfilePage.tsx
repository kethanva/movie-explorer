...
const DirectorProfilePage: React.FC = () => {
  ...
  const { director, loading, error } = useDirector(Number(id));

  if (loading) return <LoadingSpinner message="Loading director profile..." fullPage />;
  if (error || !director) return <ErrorMessage message={error || 'Director not found'} onRetry={() => navigate(-1)} />;
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 3, mb: 4, flexWrap: 'wrap' }}>
        <Avatar sx={{ width: 100, height: 100, bgcolor: 'primary.dark', fontSize: '2.5rem' }}>
          {director.name[0]}
        </Avatar>
        <Box>
          <Typography variant="h4" fontWeight={700}>{director.name}</Typography>
          {director.birth_date && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.5 }}>
              <CalendarIcon fontSize="small" color="action" />
              <Typography color="text.secondary">
                Born {new Date(director.birth_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
              </Typography>
            </Box>
          )}
          {director.nationality && (
            <Chip label={director.nationality} size="small" sx={{ mt: 1 }} />
          )}
          {director.bio && (
      ...
      <Typography variant="h5" fontWeight={600} gutterBottom>
        Directed Films {director.movies && `(${director.movies.length})`}
      </Typography>

      {!director.movies || director.movies.length === 0 ? (
        <EmptyState title="No movies found" message="This director has no movies listed yet." />
      ) : (
